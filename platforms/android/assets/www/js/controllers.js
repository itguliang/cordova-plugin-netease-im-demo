...
.controller('DashCtrl', function($scope) {
  $scope.NimLogin = function() {
    NIMPlugin.login("zhangxuan", "a123456", onSuccess, onError);
    function onSuccess(){
      alert("登录成功!");
    }
  ...
  $scope.pullMessageHistory = function() {
    NIMPlugin.pullMessageHistory("zengbo","P2P", 10,false,onSuccess, onError);
    function onSuccess(res){
      alert(JSON.stringify(res));
    }
    function onError(err){
  ...
  $scope.sendTextMsg = function() {
    NIMPlugin.sendTextMsg("zengbo", "P2P","P2P文本消息发送", onSuccess, onError);
    function onSuccess(res){
      alert("发送文本成功!"+res);
    }
    function onError(code){
  ...
  $scope.sendImageMessage = function() {
    ...
    navigator.camera.getPicture(function(imageURI) {
      ...
      NIMPlugin.sendImageMessage("zengbo", "P2P",imageURI, function onSuccess(res){
        console.log(res)
        alert("发送图片成功!");
      }, function onError(code){
        alert("发送失败!错误码:" + code);
